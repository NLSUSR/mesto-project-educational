...
const Card = class {
  ...
  constructor(item, callbacks, template, constants, userId) {
    ...
    this.#myLike = item.likes.some((like) => {
  ...
  #setEventListeners = () => {
    this.#$elementTrash.addEventListener("click", () => {
      this.#deleteCallback(this, this.#cardId);
    });
    this.#$elementImage.addEventListener("click", () => {
      this.#cardCallback(this.#cardName, this.#cardLink, this.#cardOwnerName);
    });
  ...
  #createCard = () => {
    ...
    this.#$cardContainer = this.#$cardNode.querySelector(
      this.#constants.$element
    );
    this.#$elementName = this.#$cardNode.querySelector(
      this.#constants.$placeName
    );
    this.#$elementImage = this.#$cardNode.querySelector(
      this.#constants.$placeImage
    );
    this.#$elementTrash = this.#$cardNode.querySelector(
      this.#constants.$delete
    );
    this.#$elementLike = this.#$cardNode.querySelector(this.#constants.$like);
    this.#$elementLikeCounter = this.#$cardNode.querySelector(
      this.#constants.$likeCounter
    );
    ...
    this.#cardLikes.length === 0
      ...
      : (this.#$elementLike.title = `${
          "Эту карточку лайкнул(и): " + namesString
        }`);
